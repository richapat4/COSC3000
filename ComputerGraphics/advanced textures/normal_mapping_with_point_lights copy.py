...
class Player:
    ...
    def get_view_transform(self):
        ...
        eye = self.position,
        target = self.position + self.get_forwards(),
        up = self.get_up(),
        dtype = np.float32
        )
...
class Engine:
    ...
    def render(self, scene):
        ...
        glUniform3fv(
            self.cameraForwardsLocation, 1, scene.player.forwards)
        glUniform3fv(
            self.cameraRightLocation, 1, scene.player.right)
        glUniform3fv(
            self.cameraUpLocation, 1, 
            (SCREEN_HEIGHT / SCREEN_WIDTH) * scene.player.global_up)
        ...
        object = scene.objects[OBJECT_ATMOS][0]
