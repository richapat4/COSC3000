...
original_df = pd.read_csv("Project_1\\WhalingData.csv").dropna()
original_df['Nation'].replace({"USSR": "Russia"}, inplace=True)

original_df['Type'] = original_df['Type'].str.title()

locations = pd.read_csv("Project_1\\Whaling Locations1.csv")
...
df_1.to_csv('Project_1\\GeoMap.csv', index=False)
...
data = pd.read_csv('Project_1\\EndangeredLists_2.csv')
...
def update_graphs_1(selected_year,all,animate, data, n_clicks):
    ...
    bar_chart2 = px.bar(df_area_breakdown, x="Number", y="Area", color='Whale', orientation = 'h',title=f'Whaling Location Breakdown by Area {titleStr}',barmode='stack',template = ex_template,color_discrete_sequence = point_colors,log_x=True,width=None, height=None)
    ...
    if((selected_year == 1985) and (len(all) == 0)):
        ...
        bar_chart2.update_layout(autosize=True, width = 1500, height = 400)
    elif(len(all)==1):
        print('HERE for some reason')
        bar_chart2.update_layout(height = 800, width = 1500, autosize=False)
    else:
        print('GOT HERE')
        bar_chart2.update_layout(autosize=True, width = 1500, height = 500)
    ...
    if(df_heat_year.shape[1] > 7):
        ...
        heat_map.update_layout(width = 900, height = 800)
         ...
         heat_map.update_layout(width = 600, height = 600)
        # heat_map.update_layout(coloraxis_colorbar=dict(x=1.5))
...
def update_graphs_2(selected_nation,n_clicks,data):
    ...
    if(data % 2 == 0):
        ex_template = 'plotly_white'
        img = "Project_1\\images\\seabornLightBox.png"
    else:
        ex_template = 'plotly_dark'
        img = "Project_1\\images\\seabornDarkBox.png"
